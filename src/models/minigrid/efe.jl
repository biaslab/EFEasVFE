...
@model function minigrid_state_update(p_old_location, p_old_orientation, p_old_key_door_state, p_door_location, p_key_location, previous_action, fov_observation, orientation_observation, location_transition_tensor, orientation_transition_tensor, key_door_transition_tensor, observation_tensors, number_type)
    ...
    current_location ~ DiscreteTransition(old_location, location_transition_tensor, old_orientation, key_location, door_location, old_key_door_state, previous_action)
    current_key_door_state ~ DiscreteTransition(old_key_door_state, key_door_transition_tensor, old_location, old_orientation, key_location, door_location, previous_action)

    current_orientation ~ DiscreteTransition(old_orientation, orientation_transition_tensor, previous_action)

    for x in 1:7, y in 1:7
        decomposed_tensor = observation_tensors[x, y]
        fov_observation[x, y] ~ DiscreteTransition(current_location, decomposed_tensor, orientation_observation, key_location, door_location, current_key_door_state)
    end

    current_orientation ~ DiscreteTransition(orientation_observation, diageye(number_type, 4))
end

@initialization function minigrid_state_update_initialization(p_current_location, p_current_orientation, p_current_key_door_state, p_door_location, p_key_location)
...
@model function efe_minigrid_agent(p_location, p_orientation, p_key_door_state, p_key_location, p_door_location,
    ...
    for t in 1:T
        ...
        # keylocation_marginalcomponent = JointMarginalMetaComponent(location_marginalstorage, 4, 7)
        # doorlocation_marginalcomponent = JointMarginalMetaComponent(location_marginalstorage, 5, 7)
        ...
        u[t] ~ Exploration(location_transition_tensor) where {meta=JointMarginalMeta([location_marginalcomponent, orientation_marginalcomponent, key_door_state_marginalcomponent])}
        ...
        key_location_observation_marginalcomponents = JointMarginalMetaComponent[]
        door_location_observation_marginalcomponents = JointMarginalMetaComponent[]
        ...
        location[t] ~ Ambiguity(location_transition_tensor) where {meta=JointMarginalMeta(location_observation_marginalcomponents)}
        orientation[t] ~ Ambiguity(location_transition_tensor) where {meta=JointMarginalMeta(orientation_observation_marginalcomponents)}
        key_location ~ Ambiguity(location_transition_tensor) where {meta=JointMarginalMeta(key_location_observation_marginalcomponents)}
        door_location ~ Ambiguity(location_transition_tensor) where {meta=JointMarginalMeta(door_location_observation_marginalcomponents)}
        key_door_state[t] ~ Ambiguity(location_transition_tensor) where {meta=JointMarginalMeta(key_door_state_observation_marginalcomponents)}
    end
    location[end] ~ goal
