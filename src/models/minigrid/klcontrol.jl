...
@model function klcontrol_minigrid_agent(p_old_location, p_old_orientation, p_key_location, p_door_location, p_old_key_state,
    ...
    # State inference
    current_location_cache = CachingMeta()
    current_location ~ DiscreteTransition(old_location, loc_t_tensor, old_orientation, key_location, door_location, old_key_state, old_door_state, action) where {meta=current_location_cache}
    current_orientation ~ DiscreteTransition(old_orientation, ori_t_tensor, action)
    current_door_state_cache = CachingMeta()
    current_door_state ~ DiscreteTransition(old_door_state, door_t_tensor, old_location, old_orientation, door_location, old_key_state, action) where {meta=current_door_state_cache}
    key_state_cache = CachingMeta()
    current_key_state ~ DiscreteTransition(old_key_state, key_t_tensor, old_location, old_orientation, key_location, action) where {meta=key_state_cache}


    for x in 1:7, y in 1:7
        slice = observation_tensors[x, y, :, :, :, :, :, :, :]
        meta = CachingMeta()
        observations[x, y] ~ DiscreteTransition(current_location, slice, current_orientation, key_location, door_location, current_key_state, current_door_state) where {meta=meta}
    end
    ...
    for t in 1:T
        ...
        location_cache = CachingMeta()
        location[t] ~ DiscreteTransition(previous_location, loc_t_tensor, previous_orientation, key_location, door_location, previous_key_state, previous_door_state, u[t]) where {meta=location_cache}
        orientation[t] ~ DiscreteTransition(previous_orientation, ori_t_tensor, u[t])
        door_state_cache = CachingMeta()
        door_state[t] ~ DiscreteTransition(previous_door_state, door_t_tensor, previous_location, previous_orientation, door_location, previous_key_state, u[t]) where {meta=door_state_cache}
        key_state_cache = CachingMeta()
        key_state[t] ~ DiscreteTransition(previous_key_state, key_t_tensor, previous_location, previous_orientation, key_location, u[t]) where {meta=key_state_cache}
